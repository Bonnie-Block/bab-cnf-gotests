...
func defineConnection(serverPort int) net.PacketConn {
	packetConnection, err := net.ListenPacket("udp", fmt.Sprintf("0.0.0.0:%d", serverPort))
	if err != nil {
		log.Print(err)
		os.Exit(1)
	}
...
func RunMulticastUDPServer(
	serverPort int, serverIP string, protocolVersion int, udpDatagramSize int, interfaceName string) {
	runGenericUDPServer("multicast", serverPort, serverIP, protocolVersion, udpDatagramSize, interfaceName)
}

func runGenericUDPServer(
	mode string, serverPort int, serverIP string, protocolVersion int, udpDatagramSize int, interfaceName string) {
	var testString string

	raddr, err := net.ResolveUDPAddr(
		fmt.Sprintf("%s%d", ProtocolUDP, protocolVersion),
	...
	laddr, err := net.ResolveUDPAddr(
		fmt.Sprintf("%s%d", ProtocolUDP, protocolVersion),
		fmt.Sprintf("[%s]:%d", *intFaceAddr, serverPort))
	...
	timeVal.Sec = 5
	err = syscall.SetsockoptTimeval(int(socketFile.Fd()), syscall.SOL_SOCKET, syscall.SO_SNDTIMEO, timeVal)
	...
	err = syscall.SetsockoptTimeval(int(socketFile.Fd()), syscall.SOL_SOCKET, syscall.SO_RCVTIMEO, timeVal)
	...
	if protocolVersion == 4 {
		err = syscall.SetsockoptInt(
			int(socketFile.Fd()), syscall.IPPROTO_IP, syscall.IP_MTU_DISCOVER, syscall.IP_PMTUDISC_DO)
	} else {
		err = syscall.SetsockoptInt(
			int(socketFile.Fd()), syscall.IPPROTO_IPV6, syscall.IPV6_MTU_DISCOVER, syscall.IPV6_PMTUDISC_DO)
	}
...
func RunUDPServer(serverPort int, bufferSize int) {
	packetConnection := defineConnection(serverPort)
	defer func() {
	...
	go func() {
		for {
			numberBytes, addr, err := packetConnection.ReadFrom(buffer)
			if err != nil {
				doneChan <- err
			...
			log.Printf("packet-received: bytes=%d from=%s\n",
				numberBytes, addr.String())

			deadline := time.Now().Add(20 * time.Second)
			err = packetConnection.SetWriteDeadline(deadline)
			...
			numberBytes, err = packetConnection.WriteTo(buffer[:numberBytes], addr)
