...
func modifyObjects(mode string, resDir string) error {
	...
	for _, obj := range objs {
		if mode == createMode {
			err = Apiclient.Client.Create(context.TODO(), obj)
		} else if mode == deleteMode {
			err = Apiclient.Client.Delete(context.TODO(), obj)
...
func updateObject(obj *unstructured.Unstructured) error {
	...
	return Apiclient.Client.Update(context.TODO(), obj)
}
