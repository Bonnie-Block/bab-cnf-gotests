...
func SetProperty(propertyKye, propertyValue string) ginkgo.Labels {
	return ginkgo.Label(fmt.Sprintf("polarion-parameter-%s:%s", propertyKye, propertyValue))
}
...
func appendToExistingReportFile(outputFile string, fileInfo os.FileInfo, newReport *TestSuite) {
	...
	existingTestSuiteByteFormat := make([]byte, fileInfo.Size())
	_, err = file.Read(existingTestSuiteByteFormat)
	...
	if err != nil {
		panic(fmt.Errorf("failed to unmasral existing Polarion report file: %s\n\t%w", outputFile, err))
	}
	...
	if err != nil {
		panic(fmt.Errorf("failed to generate agregated Polarion report\n\t%w", err))
	}
}
...
func generatePolarionXMLFile(outputFile string, testCases *TestSuite) {
	...
	if errors.Is(err, os.ErrNotExist) {
		...
		appendToExistingReportFile(outputFile, fileInfo, testCases)
	}
}
