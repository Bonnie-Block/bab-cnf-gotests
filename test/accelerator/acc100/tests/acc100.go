...
var _ = Describe("Intel ACC100", func() {
	...
	execute.BeforeAll(func() {
		...
		IsSriovFecDeploymentInstalled, _ := generalHelper.IsDeploymentInstalled(
			generalHelper.Apiclient, parameters.OperatorNamespace, parameters.DeploymentSriovFecName)
		...
		numberReadySriovFecDaemonsets, numberDesiredSriovFecDaemonsets := networkacceleratorhelper.CountSriovFecDaemonsets(
		...
		networkacceleratorhelper.FindAndValidateOrOverridePerformanceProfile(
			generalHelper.Apiclient, config.General.CnfNodeLabel, snoTimeoutMultiplier)

		By("Creating SriovFecClusterConfig")
		fecConfig = acc100helper.GetSriovFecAcc100ClusterConfigDefinition(
			generalHelper.Apiclient, isSingleNode)
		cnfNodelabel := strings.Split(config.General.CnfNodeLabel, "/")[1]
		networkacceleratorhelper.InstallSriovFecClusterNodeConfig(
			generalHelper.Apiclient, fecConfig, isSingleNode, cnfNodelabel)
	})
	...
	Context("sriov-fec", func() {
		...
		It("validation", func() {
			...
			bbdevPod := networkacceleratorhelper.CreateBbdevPod(
				generalHelper.Apiclient, parameters.TestNamespace, parameters.Acc100ResourceName, config)
			By("Running bbdev tests")
			bbdevTestResults := networkacceleratorhelper.RunBbdevTests(generalHelper.Apiclient, bbdevPod)
			...
			Expect(networkacceleratorhelper.IsBbdevFailedTests(bbdevTestResults)).To(BeFalse(), 
