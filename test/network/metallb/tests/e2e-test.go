...
var _ = Describe("CNF MetalLB", func() {
	...
	execute.BeforeAll(func() {
		...
		By("should have valid environment IP variable", func() {
			metallbEnvVar := networkmetallbhelper.CheckEnvVar()
			Expect(metallbEnvVar).To(Equal(true))
		})

		By("should have the MetalLB Operator deployment is installed", func() {
			isDeploymentInstalled, err := generalHelper.IsDeploymentInstalled(generalHelper.Apiclient, parameters.MetalLBOperatorNameSpace, parameters.MetalLBOperatorDeploymentName)
		...
		By("should have the MetalLB Operator deployment in running state", func() {
			isDeploymentReady, err := generalHelper.IsDeploymentReady(generalHelper.Apiclient, parameters.MetalLBOperatorNameSpace, parameters.MetalLBOperatorDeploymentName)
			Expect(err).NotTo(HaveOccurred())
		...
		By("should have MetalLB daemonset is in running state", func() {
			daemonsetRun, daemonsetDesired := generalHelper.CountDaemonsets(generalHelper.Apiclient, parameters.MetalLBOperatorNameSpace, parameters.MetalLBDaemonsetName)
	...
	It("Validate MetalLB Layer 2 functionality", func() {
		By("validate arping", func() {
			testPod := networkmetallbhelper.MLBTestPod(nodeListString[0], parameters.DefaultNameSpace, Config.Network.TestContainerImage)
			// https://bugzilla.redhat.com/show_bug.cgi?id=1987445 - fix in 4.10
			// Verifies that only one node answers the arp request
			output := networkmetallbhelper.Arping(*testPod, Config.GetMetallbEnvVar()[0])
			lineCount := 0
			...
			Expect(string(strings.Join(output, "\n"))).Should(ContainSubstring(strings.ToUpper(nodeMac)), "ARP request was not recieved from the announcing node")
			Expect(err).ToNot(HaveOccurred())

		})
		By("validate curl", func() {
			testPod := networkmetallbhelper.MLBTestPod(nodeListString[0], parameters.DefaultNameSpace, Config.Network.TestContainerImage)
			Expect(networkmetallbhelper.CurlMlbPod(*testPod, Config.GetMetallbEnvVar()[0])).Should(ContainSubstring("html"), "Curl was unable to connect to nginx")
		})
	})
})
