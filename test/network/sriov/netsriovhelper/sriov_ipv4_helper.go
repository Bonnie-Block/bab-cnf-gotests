...
func TestSriovIPv4Scenario(
	...
	if ipam == netsriovparameters.IpamWhereabouts {
		nethelper.WaitUntilIPPoolIsEmpty("192.168.100.0-30")
	}
	...
	Expect(err).ToNot(HaveOccurred())
	WaitUntilPodInStatus(clientPod, "Client", clientTestCommand, corev1.PodSucceeded, netsriovparameters.PodWaitingTime)
